...
def collate_fn(batch):
    ...
    images_grammar = batch[4]
    seqs_y_grammar = [vocab.words2indices(x) for x in batch[5]]
    ...
    max_height_gram = max(heights_grammar)
    max_width_gram = max(widths_grammar)

    x = torch.zeros(n_samples, 1, max_height_x, max_width_x)
    x_mask = torch.ones(n_samples, max_height_x, max_width_x, dtype=torch.bool)

    x_grammar = torch.zeros(n_samples, 1, max_height_gram, max_width_gram)
    x_mask_grammar = torch.ones(n_samples, max_height_x, max_width_gram, dtype=torch.bool)
