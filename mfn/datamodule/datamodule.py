...
def data_iterator(
    ...
    biggest_image_size2 = 300000

    data.sort(key=lambda x: x[1].shape[0] * x[1].shape[1],reverse=True) #排列

    i = 0
    for fname, fea, lab, fea_gram, lab_gram in data:
        size = fea.shape[0] * fea.shape[1]
        #size = fea_gram.shape[0] * fea_gram.shape[1] + fea.shape[0] * fea.shape[1]
        ...
        #elif is_train and size > biggest_image_size2:
        elif size > biggest_image_size2:
            print(
                f"image: {fname} size {size}: {fea.shape[0]} x {fea.shape[1]} =  bigger than {biggest_image_size2}, ignore"
            )
            print(
                f"image_gram: {fname} size: {fea_gram.shape[0]} x {fea_gram.shape[1]} =  bigger than {biggest_image_size2}, ignore"
            )
        else:
...
def collate_fn(batch):
    ...
    images_grammar = batch[3]
    seqs_y_grammar =  [vocab.words2indices(x) for x in batch[4]]
    ...
    max_height_img = max(heights_x)  ### 为什么要加上grammar的高度
    max_height_grm = max(heights_grammar)  ### 为什么要加上grammar的高度


    max_width = max(widths_x + widths_grammar)     ### 感觉没必要，取两者的最大就行了。

    x = torch.zeros(n_samples, 1, max_height_img, max_width)
    x_mask = torch.ones(n_samples, max_height_img, max_width, dtype=torch.bool)

    x_grammar = torch.zeros(n_samples, 1, max_height_grm, max_width)
    x_mask_grammar = torch.ones(n_samples, max_height_grm, max_width, dtype=torch.bool)
