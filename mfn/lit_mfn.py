...
from mfn.datamodule import Batch, vocab
from mfn.model.mfn import MFN
from mfn.utils.utils import (
    ExpRateRecorder, Hypothesis, ce_loss, to_bi_tgt_out, plicit_tgt_out)

def check_and_empty_cuda_cache():
    # 获取当前设备的已分配内存和缓存中的内存
    ...
    print(f"Reserved Memory (Cache): {reserved_memory:.2f} GB")
    
    # 如果缓存容量大于 24 GB，则清空缓存
    if reserved_memory > 22:
        print("Cache exceeds 22GB. Running cuda.empty_cache()...")
        torch.cuda.empty_cache()

class LitMFN(pl.LightningModule):
    def __init__(
        self,
        d_model: int,
        # encoder
    ...
    def forward(
        self, img: FloatTensor, img_mask: LongTensor, tgt: LongTensor,img_gram:FloatTensor,img_gram_mask:LongTensor
    ) -> FloatTensor:
        """run img and bi-tgt
        ...
        FloatTensor
            [2b, l, vocab_size]
        """
        return self.mfn_model(img, img_mask, tgt, img_gram, img_gram_mask)

    def training_step(self, batch: Batch, batch_idx):
        #print(f"Training on {batch.img_bases}")
        ...
        implicit_tgt, implicit_out = plicit_tgt_out(
            batch.indices, self.device, is_explicit=False, is_implicit=False)
        # implicit_tgt, implicit_out = plicit_tgt_out(
        #     batch.indices_grammar, self.device, is_explicit=False, is_implicit=False, is_grammer = True)
        
        exp_out_hat, imp_out_hat, fusion_out_hat = self(
            batch.imgs, batch.mask, exp_tgt, batch.imgs_grammar, batch.grammar_mask)

        exp_loss      = ce_loss(exp_out_hat, exp_out)
        implicit_loss = ce_loss(imp_out_hat, implicit_out, need_weight=self.hparams.dynamic_weight)
        fusion_loss   = ce_loss(fusion_out_hat, fusion_out)

        self.log("tr_i_l", implicit_loss,
                 on_step=True, on_epoch=True, sync_dist=True, prog_bar=True)
        self.log("tr_e_l", exp_loss,
                 on_step=True, on_epoch=True, sync_dist=True, prog_bar=True)
        self.log("tr_f_l", fusion_loss,
                 on_step=True, on_epoch=True, sync_dist=True, prog_bar=True)
        ...
        if batch_idx % 150 == 0:
            check_and_empty_cuda_cache()
            #torch.cuda.empty_cache()
        return loss

    def validation_step(self, batch: Batch, batch_idx):
        #print(f"Validating on {batch.img_bases}")
        fusion_tgt, fusion_out = plicit_tgt_out(
            batch.indices, self.device, is_explicit=False, is_implicit=False)
        exp_tgt, exp_out = plicit_tgt_out(
            batch.indices, self.device, is_explicit=False, is_implicit=False)
        ...
        hyps = self.approximate_joint_search(batch.imgs, batch.mask, 
                                             batch.imgs_grammar, batch.grammar_mask)
    ...
    def test_step(self, batch: Batch, _):
        print(f"Testing on {batch.img_bases}")
        hyps = self.approximate_joint_search(batch.imgs, batch.mask, 
                                             batch.imgs_grammar, batch.grammar_mask)
        self.exprate_recorder([h.seq for h in hyps], batch.indices)
    ...
    def approximate_joint_search(
        self, img: FloatTensor, mask: LongTensor, img_format: FloatTensor, mask_format: LongTensor
    ) -> List[Hypothesis]:
        return self.mfn_model.beam_search(img, mask, img_format, mask_format, **self.hparams)
    ...
    def configure_optimizers(self):
        ...
        scheduler = {
            ...
            "interval": "epoch",  # 在每个 epoch 更新
            "frequency": 1,
        }

        # 返回 optimizer 和 StepLR 学习率调度器
        return {"optimizer": optimizer, "lr_scheduler": scheduler}

    # def on_epoch_end(self): ### 这是我自己添加的hook，暂时不用
    #     # 在训练到一定 epoch 时，切换到 ReduceLROnPlateau
    #     if self.current_epoch > 3:  # 假设在第 5 个 epoch 后切换
    #         optimizer = self.optimizers()[0]
    #         reduce_scheduler = optim.lr_scheduler.ReduceLROnPlateau(
    #             optimizer,
    #             mode="max",  # 这里假设您监控的是损失的最小化
    #             factor=0.25,
    #             patience=self.hparams.patience // self.trainer.check_val_every_n_epoch,
    #         )

    #         # 更新学习率调度器
    #         self.lr_schedulers().clear()  # 清除现有调度器
    #         self.lr_schedulers().append({
    #             'scheduler': reduce_scheduler,
    #             'interval': 'epoch',  # 按 epoch 更新
    #             'monitor': 'val_ExpRate',  # 监控验证集的损失（也可以是其他指标）
    #             'frequency': self.trainer.check_val_every_n_epoch,
    #             'strict': True,
    #         })
