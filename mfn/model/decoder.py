...
from mfn.datamodule import vocab
from mfn.model.pos_enc import WordPosEnc
from mfn.model.transformer.arm import AttentionRefinementModule
from mfn.model.transformer.transformer_decoder import (
    TransformerDecoder,
    TransformerDecoderLayer,
)
from mfn.utils.generation_utils import DecodeModel
...
class Decoder(DecodeModel):
    ...
    def forward(
        ...
        """
        _, l = tgt.size()
        tgt_mask = self._build_attention_mask(l) # This is an upper triangular matrix with nonzeros values equal to 1
        tgt_pad_mask = tgt == vocab.PAD_IDX

        tgt = self.word_embed(tgt)  # [b,l,vocab_size] --> [b, l, d]
        tgt = self.pos_enc(tgt)  # [b, l, d]--> [b, l, d]
        tgt = self.norm(tgt) # [b, l, d]--> [b, l, d]

        h = src.shape[1]  # src is fused_feature
        src = rearrange(src, "b h w d -> (h w) b d") # [b, h, w, d] --> [b, h*w, d] --> [h*w, b, d] fused_feature 变成向量了
        src_mask = rearrange(src_mask, "b h w -> b (h w)") # [b, h, w] --> [b, h*w] --> [b, h*w] 变成向量了
        tgt = rearrange(tgt, "b l d -> l b d") # [b, l, d] --> [l, b, d] 为啥要这么改变维度呢？可能就是为了迁就下面的attention model吧
        ...
        #print("tgt.shape = ", tgt.shape, "out shape = ", out.shape, "src shape = ", src.shape)
        ### src is fused_feature, src_mask is fused_mask， model处理的对象实际上是tgt，基于tgt生成新的预测值out

        exp_out = rearrange(out, "l b d -> b l d") # [l, b, d] --> [b, l, d] Transformer的结果又变回来了
        imp_out = self.SCCM(exp_out, tgt_mask, tgt_pad_mask)  ### 这里处理的是logits，transformer预测结果
...
class SCCM(nn.Module):
    ...
    def forward(self, out: FloatTensor, tgt_mask: LongTensor, src_key_padding_mask: LongTensor):
        ...
        FloatTensor
            [b, l, d]
        """
        out = rearrange(out, "b t d -> t b d") ### 好像transformer对数据的batch维度要求在第二维
        out = self.te(
