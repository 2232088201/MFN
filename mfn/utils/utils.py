from typing import List, Optional, Tuple, Union

import torch
import torch.nn.functional as F
from mfn.datamodule import vocab
from einops import rearrange
from torch import LongTensor
from torchmetrics import Metric


class Hypothesis:
    seq: List[int]
    score: float

    def __init__(
        self,
        seq_tensor: LongTensor,
        score: float,
        direction: str,
    ) -> None:
        assert direction in {"l2r", "r2l"}
        raw_seq = seq_tensor.tolist()

        if direction == "r2l":
            result = raw_seq[::-1]
        else:
            result = raw_seq

        self.seq = result
        self.score = score

    def __len__(self):
        if len(self.seq) != 0:
            return len(self.seq)
        else:
            return 1

    def __str__(self):
        return f"seq: {self.seq}, score: {self.score}"


class ExpRateRecorder(Metric):
    def __init__(self, dist_sync_on_step=False):
        super().__init__(dist_sync_on_step=dist_sync_on_step)

        self.add_state("total_line", default=torch.tensor(
            0.0), dist_reduce_fx="sum")
        self.add_state("rec", default=torch.tensor(0.0), dist_reduce_fx="sum")

    def update(self, indices_hat: List[List[int]], indices: List[List[int]]):
        for pred, truth in zip(indices_hat, indices):
            pred = vocab.indices2label(pred)
            truth = vocab.indices2label(truth)

            is_same = pred == truth

            if is_same:
                self.rec += 1

            self.total_line += 1

    def compute(self) -> float:
        if self.total_line < 1:
            self.total_line = 1
        exp_rate = self.rec / self.total_line
        return exp_rate


def smooth_weight_adjustment(targets, class_of_interest=3, base_weight=1.0, max_weight=10.0):
    """
    Adjust weights based on frequency smoothing, using a logarithmic function for adjustment.
    Args:
        frequency: (torch.Tensor): [batch, len]
    Returns:
        torch.Tensor: adjusted weight
    """
    class_count = torch.sum(targets == class_of_interest)
    total_count = torch.numel(targets)
    frequency = class_count.float() / total_count

    # use a logarithmic function
    weight = base_weight + torch.log1p(1 / ((1-frequency) + 1e-6))

    # limit to max weight
    weight = torch.clamp(weight, max=max_weight)
    return weight


def ce_loss(
    output_hat: torch.Tensor,
    output: torch.Tensor,
    ignore_idx: int = vocab.PAD_IDX,
    reduction: str = "mean",
    need_weight: bool = False,
    class_of_interest: int = 3,
) -> torch.Tensor:
    """comput cross-entropy loss

    Args:
        output_hat (torch.Tensor): [batch, len, e]
        output (torch.Tensor): [batch, len]
        ignore_idx (int):

    Returns:
        torch.Tensor: loss value
    """
    flat_hat = rearrange(output_hat, "b l e -> (b l) e")
    flat = rearrange(output, "b l -> (b l)")
    weights = None
    if need_weight:
        weights = torch.ones(len(vocab), device=output_hat.device)
        smooth_weight = smooth_weight_adjustment(flat)
        weights[:] = smooth_weight
        weights[class_of_interest] = 1.0

    loss = F.cross_entropy(flat_hat, flat, weight=weights,
                           ignore_index=ignore_idx, reduction=reduction)
    return loss


def to_tgt_output(
    tokens: Union[List[List[int]], List[LongTensor]],
    direction: str,
    device: torch.device,
    pad_to_len: Optional[int] = None,
    is_explicit: bool = False,
    is_implicit: bool = False,
    is_grammer: bool = False,
) -> Tuple[LongTensor, LongTensor]:
    """Generate tgt and out for indices

    Parameters
    ----------
    tokens : Union[List[List[int]], List[LongTensor]]
        indices: [b, l]
    direction : str
        one of "l2f" and "r2l"
    device : torch.device

    Returns
    -------
    Tuple[torch.Tensor, torch.Tensor]
        tgt, out: [b, l], [b, l]
    """
    assert direction in {"l2r", "r2l"}

    if isinstance(tokens[0], list):
        tokens = [torch.tensor(t, dtype=torch.long) for t in tokens]

    if is_implicit:
        filtered_tokens = []
        for token in tokens:
            #mask = (token != vocab.word2idx['{']) & (token != vocab.word2idx['}']) & (token != vocab.word2idx['^']) & (token != vocab.word2idx['_'])
            mask1 = (token != vocab.word2idx['{']) & (token != vocab.word2idx['}']) & (token != vocab.word2idx['^']) & (token != vocab.word2idx['_'])
            mask2 = (token != vocab.word2idx['[']) & (token != vocab.word2idx[']']) & (token != vocab.word2idx['\\frac']) & (token != vocab.word2idx['-'])
            mask3 = (token != vocab.word2idx['(']) & (token != vocab.word2idx[')']) & (token != vocab.word2idx[',']) & (token != vocab.word2idx['!'])
            mask4 = (token != vocab.word2idx['.']) & (token != vocab.word2idx['/']) & (token != vocab.word2idx['.']) & (token != vocab.word2idx['!'])
            mask = mask1 & mask2 & mask3 & mask4
            #tokenchar = vocab.indices2words(token)
            # print("the mask", mask,'and the token', token)
            token[mask] = 3 #vocab.PAD_IDX
            filtered_tokens.append(token)
        tokens = filtered_tokens

    if is_explicit:
        #针对特定符号 {, }, ^, _，对 tokens 中不包含这些符号的部分进行修改（替换为 3）
        filtered_tokens = []
        for token in tokens:
            mask = (token == vocab.word2idx['{']) | (token == vocab.word2idx['}']) | (
                token == vocab.word2idx['^']) | (token == vocab.word2idx['_'])
            token[mask] = 3
            filtered_tokens.append(token)
        tokens = filtered_tokens
    # if is_grammer:
    #     filtered_tokens = []
    #     for token in tokens:
    #         #print(token)
    #         mask = (token == vocab.word2idx['#'])
    #         token[mask] = 3  ### 置空值
    #         #print(token)
    #         filtered_tokens.append(token)
    #     tokens = filtered_tokens
    #print(len(tokens))
    if direction == "l2r":
        tokens = tokens
        start_w = vocab.SOS_IDX
        stop_w = vocab.EOS_IDX
    else:
        tokens = [torch.flip(t, dims=[0]) for t in tokens]
        start_w = vocab.EOS_IDX
        stop_w = vocab.SOS_IDX

    batch_size = len(tokens)
    lens = [len(t) for t in tokens]

    length = max(lens) + 1
    if pad_to_len is not None:
        length = max(length, pad_to_len)

    tgt = torch.full(
        (batch_size, length),
        fill_value=vocab.PAD_IDX,
        dtype=torch.long,
        device=device,
    )
    out = torch.full(
        (batch_size, length),
        fill_value=vocab.PAD_IDX,
        dtype=torch.long,
        device=device,
    )

    for i, token in enumerate(tokens):
        tgt[i, 0] = start_w
        tgt[i, 1: (1 + lens[i])] = token

        out[i, : lens[i]] = token
        out[i, lens[i]] = stop_w

    return tgt, out


def to_bi_tgt_out(
    tokens: List[List[int]], device: torch.device
) -> Tuple[LongTensor, LongTensor]:
    """Generate bidirection tgt and out

    Parameters
    ----------
    tokens : List[List[int]]
        indices: [b, l]
    device : torch.device

    Returns
    -------
    Tuple[LongTensor, LongTensor]
        tgt, out: [2b, l], [2b, l]
    """
    l2r_tgt, l2r_out = to_tgt_output(tokens, "l2r", device)
    r2l_tgt, r2l_out = to_tgt_output(tokens, "r2l", device)

    tgt = torch.cat((l2r_tgt, r2l_tgt), dim=0)
    out = torch.cat((l2r_out, r2l_out), dim=0)

    return tgt, out


def plicit_tgt_out(
    tokens: List[List[int]], device: torch.device, is_implicit: bool = False, is_explicit: bool = False, is_grammer: bool = False,
) -> Tuple[LongTensor, LongTensor]:
    """Generate bidirection tgt and out

    Parameters
    ----------
    tokens : List[List[int]]
        indices: [b, l]
    device : torch.device

    Returns
    -------
    Tuple[LongTensor, LongTensor]
        tgt, out: [2b, l], [2b, l]
    """
    l2r_tgt, l2r_out = to_tgt_output(
        tokens, "l2r", device, is_explicit=is_explicit, is_implicit=is_implicit,is_grammer = is_grammer)
    r2l_tgt, r2l_out = to_tgt_output(
        tokens, "r2l", device, is_explicit=is_explicit, is_implicit=is_implicit,is_grammer = is_grammer)

    tgt = torch.cat((l2r_tgt, r2l_tgt), dim=0)
    out = torch.cat((l2r_out, r2l_out), dim=0)

    return tgt, out
